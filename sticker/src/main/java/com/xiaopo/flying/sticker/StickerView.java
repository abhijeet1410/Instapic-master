...
public class StickerView extends FrameLayout {
  ...
  public void placeStickerOnBack(Sticker sticker){
      stickers.remove(sticker);
//      Sticker temp = getResizedSticker(sticker);
//      stickers.add(0, temp);
      stickers.add(0, sticker);
      invalidate();
  }

  public void moveStickerToCenter(Sticker sticker){
      setStickerPosition(sticker, Sticker.Position.CENTER);
      invalidate();
  }

    private Sticker getResizedSticker(Sticker sticker) {
        sizeMatrix.reset();
        ...
        sizeMatrix.postScale(scaleFactor , scaleFactor , width , height);
    ...
    for (int i = stickers.size() - 1; i >= 0; i--) {
      if (isInStickerArea(stickers.get(i), downX, downY)) {
        return stickers.get(i);
          ...
          onStickerOperationListener.onOutSideStickerClicked();
      }
    }
    return null;
  }
  ...
  public void save(@NonNull File file, Bitmap bitmap) {
    try {
